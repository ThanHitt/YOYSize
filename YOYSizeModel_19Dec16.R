...
# Add YOY abundance
yoy <- summaryBy(idx ~ VisitID, data=d[d$TL_mm<101,], FUN=sum) # YOY abundance
yoy$idx.sum <- (yoy$idx.sum - mean(yoy$idx.sum))/sd(yoy$idx.sum) # z-scores by grand mean and sd
colnames(yoy) <- c("VisitID","yoyN")
d <- merge(d, yoy, by="VisitID", all=F) # limit dataset to samples where YOY were observed

# Add adult abundance
adu <- summaryBy(idx ~ VisitID, data=d[d$TL_mm>100,], FUN=sum) # adult abundance
d1 <- merge(d, adu, by="VisitID", all=T)
d1$idx.sum[is.na(d1$idx.sum)] <- 0 # indicate where no adults were observed
adu1 <- summaryBy(idx.sum ~ VisitID, data=d1, FUN=mean)
...
dy <- droplevels(subset(d, TL_mm < 101))
nInds <- nrow(dy)
...
sa <- droplevels(subset(sa, SiteID %in% unique(d$SiteID)))
si <- read.csv("SiteCovs.csv", header=T, sep=",") # Site-level covariates
si <- droplevels(subset(si, SiteID %in% unique(d$SiteID)))
...
yN <- summaryBy(yoyN ~ SiteID + Year + VisitID, data=dy, FUN=mean)
...
aN <- summaryBy(aduN ~ SiteID + Year + VisitID, data=dy, FUN=mean)
...
cat("
    ...
    for(j in 1:J){
      # Random site intercept on mean [j=sites]
      site.ran.mu[j] ~ dnorm(site.hat[j], tau.site.mu)
      site.hat[j] <- b10*elev[j] + b11*uba[j] + b12*grad[j]
      site.ran.sigma[j] ~ dnorm(site.ran.sig.mu, tau.site.sigma)
      ...
      for(m in 1:M){
        ...
        mu.sigma[j,m] <- Mu.Sigma + site.ran.sigma[j] + b13*doy1[j,m] + b14*yoyN1[j,m] + b15*aduN1[j,m] + b16*fp1[j,m] + 
                         ...
                         b22*elev[j] + b23*uba[j] + b24*grad[j]
      }
    }
...
dat <- list(y = dy$TL_mm,
            ...
            sigid = as.numeric(as.factor(unique(dy$SiteID))),
            n = nInds,
            J = nSites,
            M = nYears,
...
"b17","b18","b19","b20","b21","b22","b23","b24","alpha","sig.site","sigma","site.ran.sigma","site.ran.mu")
